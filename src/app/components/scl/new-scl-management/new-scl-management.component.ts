...
@Component({
  selector: 'app-new-scl-management',
  templateUrl: './new-scl-management.component.html',
  styleUrls: ['./new-scl-management.component.css']
})
export class NewSclManagementComponent implements OnInit,ComponentCanDeactivate {
  ...
  readGroupsList() {
    this.managementService.getManagementGroupList(3)
      .subscribe(
        item => {
  ...
  readWorkFlowList() {
    this.managementService.getWorkflowList(3)
      .subscribe(
        item => {
  ...
  managementAdd(){
    let data:any={NombreGestion:this.f.managementName.value,IdGrupo:this.groupSelected.id,IdFlujo:this.workFlowSelected.id,IdTipoOperacion:3,GestionCalendario:this.dataCalendar};
    this.managementService.addManagement(data)
      .subscribe(
        respuesta => {
          ...
          this.modalService.dismissAll();
          this.router.navigate(['/manager/scl']);
        }, error => {
          if (error['statusText'] == 'Unauthorized' && error['status'] == 401){             
            this.loginService.clearSessionLogin();
            this.router.navigate(['/login']);
