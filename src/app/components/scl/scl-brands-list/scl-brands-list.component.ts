...
@Component({
  selector: 'app-scl-brands-list',
  templateUrl: './scl-brands-list.component.html',
  styleUrls: ['./scl-brands-list.component.css']
})
export class SclBrandsListComponent implements OnInit, ComponentCanDeactivate {
  ...
  readBrandsList() {
    this.manageBrandsService.getBrandsList(3)
      .subscribe(
        item => {
  ...
  deleteBrand(g: any) {
    var suscripcion = this.manageBrandsService.deleteBrand(g.brand, 3)
      .subscribe(
        respuesta => {
          if (respuesta["State"]) {
  ...
  editBrand(IdBrand: string) {
    this.router.navigate(['/manage-brands/scl/updateBrand/' + IdBrand]);
  }
