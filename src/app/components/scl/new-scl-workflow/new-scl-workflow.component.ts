...
@Component({
  selector: 'app-new-scl-workflow',
  templateUrl: './new-scl-workflow.component.html',
  styleUrls: ['./new-scl-workflow.component.css']
})
export class NewSclWorkflowComponent implements OnInit,ComponentCanDeactivate {
  ...
  ngOnInit() {
    ...
    let wf: any = { Creacion:true,NombreFlujo: null, AnchoPx: x, AltoPx: y, IdTipoOperacion: 3, Simbolos: [] }
    ...
    this.symbolsService.saveWorkflow(data)
      .subscribe(
        respuesta => {
          ...
          if(workFlowCreated){
            this.symbolsService.getRefWorkflow(2,this.f.workflowName.value,3)
            .subscribe(
              respuesta => {
                if (respuesta.hasOwnProperty('ObtenerReferenciaFlujoResult')) {
                  let elementWF:any = respuesta['ObtenerReferenciaFlujoResult'];
                  let r:any='/workflow/scl/edit-workflow/'+elementWF["Id"];
