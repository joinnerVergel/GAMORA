...
@Component({
  selector: 'app-scl-sub-categories-list',
  templateUrl: './scl-sub-categories-list.component.html',
  styleUrls: ['./scl-sub-categories-list.component.css']
})
export class SclSubCategoriesListComponent implements OnInit, OnDestroy, ComponentCanDeactivate {
        ...
        item => {
          ...
          if (item.hasOwnProperty('CategoriaPorIdResult')) {
            ...
            if(this.categorySelected.nameCategory==null){
              ...
              this.router.navigate(['/events-manager/scl']);
            }else{
  ...
  editSubcategory(IdSubcategory: number) {
    if (this.readVisibilityActions('CONSULTAR_ELEMENTO')) {
      this.router.navigate(['/events-manager/scl/category/' + this.categorySelected.idCategory + '/sub-category/' + IdSubcategory])
    }
  }
