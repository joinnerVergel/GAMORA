...
@Component({
  selector: 'app-scl-workflow-list',
  templateUrl: './scl-workflow-list.component.html',
  styleUrls: ['./scl-workflow-list.component.css']
})
export class SclWorkflowListComponent implements OnInit,ComponentCanDeactivate {
  ...
  readWorkFlowList() {
    this.symbolsService.getWorkflowList(3)
      .subscribe(
        item => {
  ...
  editWorkflow(workFlowId: number) {
    this.router.navigate(['/workflow/scl/edit-workflow/' + workFlowId])
  }
