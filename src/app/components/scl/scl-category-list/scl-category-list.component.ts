...
@Component({
  selector: 'app-scl-category-list',
  templateUrl: './scl-category-list.component.html',
  styleUrls: ['./scl-category-list.component.css']
})
export class SclCategoryListComponent implements OnInit, ComponentCanDeactivate {
  ...
  addCategory() {
    if (this.readVisibilityActions('CREAR_CATEGORIA')) {
      let data: any = { CategoriaNombre: this.f.categoryName.value, CreadoPor: "SYSTEM", IdTipoOperacion: 3 };
  ...
  readCategoriesList() {
    this.eventsService.getCategoriesList(3)
      .subscribe(
        item => {
  ...
  editSubcategories(categoryId: number) {
    if (this.readVisibilityActions('CONSULTAR_SUBCATEGORIA')) {
      this.router.navigate(['/events-manager/scl/category/' + categoryId])
    }
  }
