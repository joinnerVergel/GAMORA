...
@Component({
  selector: 'app-davox-manager-list',
  templateUrl: './davox-manager-list.component.html',
  styleUrls: ['./davox-manager-list.component.css']
})
export class DavoxManagerListComponent implements OnInit, ComponentCanDeactivate {
  ...
  readManagementList() {
    let x: number = 1;
    this.managementService.getManagementList(4)
      .subscribe(
        item => {
  ...
  dataChange() {
    if (this.readVisibilityActions('EDITAR_GESTION')) {
      ...
      let data: any = { idGestion: x.idManagement, transaccion: x.state, IdTipoOperacion: 4 }
