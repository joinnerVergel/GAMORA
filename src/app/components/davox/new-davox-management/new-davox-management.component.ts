...
@Component({
  selector: 'app-new-davox-management',
  templateUrl: './new-davox-management.component.html',
  styleUrls: ['./new-davox-management.component.css']
})
export class NewDavoxManagementComponent implements OnInit,ComponentCanDeactivate {
  ...
  readGroupsList() {
    this.managementService.getManagementGroupList(4)
      .subscribe(
        item => {
  ...
  readWorkFlowList() {
    this.managementService.getWorkflowList(4)
      .subscribe(
        item => {
  ...
  managementAdd(){
    let data:any={NombreGestion:this.f.managementName.value,IdGrupo:this.groupSelected.id,IdFlujo:this.workFlowSelected.id,IdTipoOperacion:4,GestionCalendario:this.dataCalendar};
    this.managementService.addManagement(data)
      .subscribe(
        respuesta => {
          ...
          this.modalService.dismissAll();
          this.router.navigate(['/manager/davox']);
        }, error => {
          if (error['statusText'] == 'Unauthorized' && error['status'] == 401){             
            this.loginService.clearSessionLogin();
            this.router.navigate(['/login']);
