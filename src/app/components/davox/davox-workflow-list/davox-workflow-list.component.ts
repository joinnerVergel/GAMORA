...
@Component({
  selector: 'app-davox-workflow-list',
  templateUrl: './davox-workflow-list.component.html',
  styleUrls: ['./davox-workflow-list.component.css']
})
export class DavoxWorkflowListComponent implements OnInit,ComponentCanDeactivate {
  ...
  readWorkFlowList() {
    this.symbolsService.getWorkflowList(4)
      .subscribe(
        item => {
  ...
  editWorkflow(workFlowId: number) {
    this.router.navigate(['/workflow/davox/edit-workflow/' + workFlowId])
  }
