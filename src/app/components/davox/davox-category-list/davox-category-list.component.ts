...
@Component({
  selector: 'app-davox-category-list',
  templateUrl: './davox-category-list.component.html',
  styleUrls: ['./davox-category-list.component.css']
})
export class DavoxCategoryListComponent implements OnInit, ComponentCanDeactivate {
  ...
  addCategory() {
    if (this.readVisibilityActions('CREAR_CATEGORIA')) {
      let data: any = { CategoriaNombre: this.f.categoryName.value, CreadoPor: "SYSTEM", IdTipoOperacion: 4 };
  ...
  readCategoriesList() {
    this.eventsService.getCategoriesList(4)
      .subscribe(
        item => {
  ...
  editSubcategories(categoryId: number) {
    if (this.readVisibilityActions('CONSULTAR_SUBCATEGORIA')) {
      this.router.navigate(['/events-manager/davox/category/' + categoryId])
    }
  }
