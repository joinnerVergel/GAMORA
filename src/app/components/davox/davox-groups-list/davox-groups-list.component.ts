...
@Component({
  selector: 'app-davox-groups-list',
  templateUrl: './davox-groups-list.component.html',
  styleUrls: ['./davox-groups-list.component.css']
})
export class DavoxGroupsListComponent implements OnInit, ComponentCanDeactivate {
  ...
  readGroupsList() {
    this.manageGroupsService.getManagementGroupList(4)
      .subscribe(
        item => {
  ...
  deleteGroup(g: any) {
    if (this.readVisibilityActions('ELIMINAR_GRUPO')) {
      let FiltroGrupo: any = { idTipoOperacion: 4, NombreGrupo: g.name, EdadMoraInicial: null, EdadMoraFinal: null, PrioridadBasica: null, ListaFiltros: [], tipoTransaccion: 1003 };
      var suscripcion = this.manageGroupsService.deleteManagementGroup(FiltroGrupo)
        .subscribe(
          respuesta => {
            ...
            this.readGroupsList();
            this.router.navigate(['/management-groups/davox']);
          });
    }
  };
  ...
  updatePriorityGroup(x: any) {
    if (this.readVisibilityActions('EDITAR_GRUPO')) {
      let data: any = { idTipoOperacion: 4, NombreGrupo: x.name, EdadMoraInicial: null, EdadMoraFinal: null, PrioridadBasica: this.basicPriorityGroupSelected, ListaFiltros: [], tipoTransaccion: 1004 };
  ...
  toogleSpecialPriority(x: any) {
    if (this.readVisibilityActions('EDITAR_GRUPO')) {
      let data: any = { idTipoOperacion: 4, NombreGrupo: x.name, EdadMoraInicial: null, EdadMoraFinal: null, PrioridadBasica: null, ListaFiltros: [], tipoTransaccion: 1005 };
  ...
  isEditGroup() {
    this.manageGroupsService.getIsEditGroup(4)
      .subscribe(
        item => {
