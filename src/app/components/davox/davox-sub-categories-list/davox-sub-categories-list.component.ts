...
@Component({
  selector: 'app-davox-sub-categories-list',
  templateUrl: './davox-sub-categories-list.component.html',
  styleUrls: ['./davox-sub-categories-list.component.css']
})
export class DavoxSubCategoriesListComponent implements OnInit, OnDestroy, ComponentCanDeactivate {
        ...
        item => {
          ...
          if (item.hasOwnProperty('CategoriaPorIdResult')) {
            ...
            if(this.categorySelected.nameCategory==null){
              ...
              this.router.navigate(['/events-manager/davox']);
            }else{
  ...
  editSubcategory(IdSubcategory: number) {
    if (this.readVisibilityActions('CONSULTAR_ELEMENTO')) {
      this.router.navigate(['/events-manager/davox/category/' + this.categorySelected.idCategory + '/sub-category/' + IdSubcategory])
    }
  }
