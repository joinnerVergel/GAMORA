...
export class NewMobileWorkflowComponent implements OnInit,ComponentCanDeactivate {
  ...
  ngOnInit() {
    ...
    let wf: any = { Creacion:true,NombreFlujo: null, AnchoPx: x, AltoPx: y, IdTipoOperacion: 2, Simbolos: [] }
    ...
    this.symbolsService.saveWorkflow(data)
      .subscribe(
        respuesta => {
          ...
          if(workFlowCreated){
            this.symbolsService.getRefWorkflow(2,this.f.workflowName.value,2)
            .subscribe(
              respuesta => {
                if (respuesta.hasOwnProperty('ObtenerReferenciaFlujoResult')) {
                  let elementWF:any = respuesta['ObtenerReferenciaFlujoResult'];
                  let r:any='/workflow/mobile/edit-workflow/'+elementWF["Id"];
