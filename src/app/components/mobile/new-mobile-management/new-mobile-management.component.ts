...
export class NewMobileManagementComponent implements OnInit,ComponentCanDeactivate {
  ...
  readGroupsList() {
    this.managementService.getManagementGroupList(2)
      .subscribe(
        item => {
  ...
  readWorkFlowList() {
    this.managementService.getWorkflowList(2)
      .subscribe(
        item => {
  ...
  managementAdd(){
    let data:any={NombreGestion:this.f.managementName.value,IdGrupo:this.groupSelected.id,IdFlujo:this.workFlowSelected.id,IdTipoOperacion:2,GestionCalendario:this.dataCalendar};
    this.managementService.addManagement(data)
      .subscribe(
        respuesta => {
          ...
          this.modalService.dismissAll();
          this.router.navigate(['/manager/mobile']);
        }, error => {
          if (error['statusText'] == 'Unauthorized' && error['status'] == 401){             
            this.loginService.clearSessionLogin();
            this.router.navigate(['/login']);
